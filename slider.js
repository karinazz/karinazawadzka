class Slider {
  constructor(selector) {
    window.MainSlider = window.MainSlider || window.jQuery;
    this.version = 3.1;
    this.settings = {
      classActive: "active",
      classFirst: "first",
      ...
      classWait: "wait",
      classDirectionUp: "direction-up",
      classDirectionDown: "direction-down",
    ...
    this.library = class Library {
      ...
      $(selector) {
        return new MainSlider(selector);
      }
      ...
      load(callback, type, index, debug) {
        ...
        if(img) {
          img.mainSliderStack = img.mainSliderStack || [];
          this.loading();
          this.slider.point(index, "loading");
          if(img.mainSliderStack.length === 0) {
            img.src = src;
            img.mainSliderStack.push({
              object: this,
              img: img,
              callback: callback,
              type: type,
  ...
  rwd() {
    this.resize = mainSlider.debounce(() => {
      this.responsive();
    }, 200);
  ...
  mainSlider() {
    window.mainSlider = window.mainSlider || {};

    mainSlider.throttle = mainSlider.throttle || ((callback, delay) => {
      let throttle;
      return (...args) => {
        if (!throttle) {
          callback(...args);
    ...
    mainSlider.debounce = mainSlider.debounce || ((callback, delay) => {
      let timeout;
      return (...args) => {
        const that = this;
  ...
  boot(query) {
    this.$slider = new MainSlider(query || this.settings.selector);
    this.slider = this.factory(this.$slider);
    this.len = this.$slider.length;

    this.$parent = new MainSlider(this.settings.parent || ((this.slider.element) ? this.slider.element.parentNode : false));
